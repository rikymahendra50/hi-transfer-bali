    ...
    <div class="space-y-2">
      ...
      <div v-if="arrayChange" class="space-y-4">
        ...
        <Btn
          ...
          :class="{ 'animate-bounce': arrayChange }"
...
async function uploadImageToServer(image) {
  ...
  const { data, error } =
    ...
    CommonResponse <
    ImageResponse >>
      (`/admins/products/${props.productSlug}/images?lang=${locale.value}`,
      {
        method: "POST",
        body: formData,
        ...requestOptions,
...
async function saveImagesChangePosition() {
  ...
  const { error } = await useFetch(
    `/admins/products/${props.productSlug}/save-images?lang=${locale.value}`,
    {
      method: "POST",
      body: {
        images: images.value.map((el) => el.id),
      },
      ...requestOptions,
    }
  );
